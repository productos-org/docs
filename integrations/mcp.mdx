---
title: "MCP"
description: "What the Model Context Protocol is, why ProductOS integrates over it, and what your project exposes to connected clients — from live context to the full 55-tool coding surface."
...
ProductOS uses MCP to expose your project as a live context source — and, beyond context, as a full remote development environment. Instead of pasting your PRD into a chat window and hoping it sticks, you connect the tool once, and it works against the actual product context.
...
Context is only part of the connection. The ProductOS MCP server also exposes your project's cloud sandbox — files, a shell, sandbox lifecycle, databases and storage, GitHub sync, deployments, a headless browser, mobile publishing, and multi-surface Product orchestration. That's **55 tools**, each gated behind the scope you grant the connection (`project:read`, `project:write`, or `project:exec`), so a connected agent can go from reading your spec to editing files, running builds, and shipping to production — all remote, with no local clone.

See the [MCP tools reference](/integrations/mcp-tools) for the complete list, grouped by category with the scope each tool requires.
