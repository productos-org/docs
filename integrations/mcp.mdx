...
Context is only part of the connection. The ProductOS MCP server also exposes your project's cloud sandbox — files, a shell, sandbox lifecycle, databases and storage, GitHub sync, deployments, a headless browser, mobile publishing, and multi-surface Product orchestration. That's **57 tools**, each gated behind the scope you grant the connection (`project:read`, `project:write`, or `project:exec`), so a connected agent can go from reading your spec to editing files, running builds, and shipping to production — all remote, with no local clone.
